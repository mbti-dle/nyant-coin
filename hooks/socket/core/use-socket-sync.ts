...
export const useSocketSync = <T>() => {
  ...
  const handleGameStateSync = useCallback(
    (socket: Socket, getGameData: () => { gameId: string | null; playerId: string | null }) => {
  ...
  const handleSyncComplete = useCallback((gameSnapshot: T) => {
  ...
  const clearReconnectionState = useCallback(() => {
    reconnectionInProgress.current = false
    gameRestoreToastShown.current = false
  }, [])
